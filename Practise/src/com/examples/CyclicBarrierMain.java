...
class Task implements Runnable {
	...
	public void run() {
		try {
			...
			System.out.println(Thread.currentThread().getName() + " has crossed the barrier" + LocalDateTime.now());
			barrier.await();
...
public class CyclicBarrierMain {
	public static void main(String args[]) {
		...
		final CyclicBarrier cb = new CyclicBarrier(3, new Runnable() {
			...
			public void run() {
				System.out.println(
						"All parties have arrived at the barrier, lets continue execution." + LocalDateTime.now());
			}
		});
